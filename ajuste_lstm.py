...
columns = ["residuals", "dew", "temp", "press", "wnd_dir", "wnd_spd", "snow", "rain"]
...
model.compile(
    loss="mse", optimizer=Adam(learning_rate=0.00001), metrics=[RootMeanSquaredError()]
)
...
test_results = pd.DataFrame(data={'Train Predictions': vitor,
                                  'Actual':y_test_vitor})
test_results.head()
